    ...
    <section id="container" class="">
        ...
        <section id="main-content">
            <section class="wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h3 class="page-header"><i class="fa fa-file-text-o"></i></i>Imagem</h3>
                        <ol class="breadcrumb">
                            <li><i class="fa fa-home"></i><a href="<?= URL_RAIZ ?>/home">Home</a></li>
                            <li><i class="fa fa-eye "></i>Ver Conteúdo</li>
                                    ...
                                    <div id="pageImagem" class="tab-pane active">
                                        <div class="profile-activity">
                                            ...
                                            <table id="tabela" class="table table-hover table-striped" data-url="<?= URL_RAIZ ?>/resources/dataTables_traducao.txt">
                                                ...
                                                <tbody id="tbody" data-url="<?= URL_RAIZ ?>/imagem">
                                                    <?php
                                                    ...
                                                    $id_usuario = $usuario_equipe->getResult()['id_usuario'];
                                                    $id_equipe = $usuario_equipe->getResult()['tb_equipe_id_equipe'];

                                                    $daoimagem = new DaoImagem();
                                                    if($id_usuario == $info[2]){
                                                        $daoimagem->showConteudoUsuarioEquipe(array('id_equipe' => $id_equipe, 'id_usuario' => $info[2]));
                                                    }else{
                                                        $daoimagem->showConteudoUsuario(array('id_usuario' => $info[2]));
                                                    }
                                                    
                                                    if ($daoimagem->getRowCount() > 0) {
                                                        foreach ($daoimagem->getResult() as $imagem) :
                                                            ?>
                                                            <tr>
                                                                <td class="jus" data-id="<?= $imagem['id_imagem']; ?>"> <?= $imagem['nome'] ?></td>
                                                                <td class="jus"> <?= $imagem['descricao'] ?></td>
                                                                <td class="jus"> <?= $imagem['fonte'] ?></td>
                                                                <td class="mod">
    ...
    $(document).ready(function() {
        ...
        $("#btn-verificar").on("click", function() {
            ...
            $.ajax({
                ...
                type: "POST",
                data: $("#form-validar").serialize() + "&action=validarImagem",
                success: function(resultado) {
                    if (resultado == "success") {
                        $("#msg").html("Imagem excluida com sucesso!");
                        ...
                        $("#aviso").html("Esta imagem pertence a outro usuário!");
                        $("#modalAviso").modal("show");
                    } else {
                        $("#aviso").html("Você não pode excluir uma imagem vinculado a uma pergunta!");
                        $("#modalAviso").modal("show");
                    }
                }
            });
        ...
        $(document).on('click', '.fa-eye', function() {
            ...
            $.ajax({
                ...
                type: "POST",
                data: "id=" + id_recuperado + "&action=viewImagem",
                success: function(resultado) {
                    if (resultado) {

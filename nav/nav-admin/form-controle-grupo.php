...
<head>
    ...
    <link rel="icon" href="<?= URL_RAIZ ?>/img/icons/gemepiicon02.png"> <!-- Custom styles -->
    ...
    <section id="container" class="">
        ...
        <section id="main-content">
            <section class="wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h3 class="page-header"><i class="fa fa-file-text-o"></i></i>Equipe</h3>
                        <ol class="breadcrumb">
                            <li><i class="fa fa-home"></i><a href="<?= URL_RAIZ ?>/home">Home</a></li>
                            <li><i class="fa fa-users"></i>Ver Equipe</li>
                                            ...
                                            <table id="tabela" class="table table-hover table-striped" data-url="<?= URL_RAIZ ?>/resources/dataTables_traducao.txt">
                                                <thead id="thead" data-url="<?= URL_RAIZ ?>/manipulacao/controle-formulario.php">
                                                    <tr>
                                                        ...
                                                        <th> Visualizar integrantes</th>
                                                        <th> Adicionar integrante</th>
                                                        <th> Opção</th>
                                                    </tr>
                                                </thead>
                                                <tbody id="tbody" data-url="<?= URL_RAIZ ?>/form-controle-grupo">
                                                    ...
                                                    $daoequipe->showEquipeUsuario($info[2]);
                                                    if ($daoequipe->getRowCount() > 0) {
                                                        foreach ($daoequipe->getResult() as $equipe) :
                                                            ?>
                                                            <tr>
                                                                <td class="jus" data-id="<?= $equipe['id_equipe']; ?>"> <?= $equipe['nome'] ?></td>


                                                                <td class="mod">
                                                                    <a href="<?= URL_RAIZ ?>/remove-usuario-equipe/<?= $equipe['id_equipe']; ?>"><i class="fa fa-users btn btn-secundare"></i></a>
                                                                </td>
                                                                <td class="mod">
                                                                    <a href="<?= URL_RAIZ ?>/add-equipe-usuario/<?= $equipe['id_equipe']; ?>"><i class="icon_plus btn btn-default"></i></a>
                                                                </td>
                                                                <td class="mod">
    ...
    $(document).ready(function() {
        ...
        $("#btn-verificar").on("click", function() {
            ...
            $.ajax({
                ...
                type: "POST",
                data: $("#form-validar").serialize() + "&action=validarEquipe",
                success: function(resultado) {
                    if (resultado == "success") {
                        $("#msg").html("Equipe excluido com sucesso!");
                        ...
                        $("#aviso").html("Você não pode excluir está equipe!");
                        $("#modalAviso").modal("show");
                    }
                }
            });
        ...
        $(document).on('click', '.fa-eye', function() {
            ...
            $.ajax({
                ...
                type: "POST",
                data: "id=" + id_recuperado + "&action=viewParticipantes",
                success: function(resultado) {
                    if (resultado) {
                        ...
                        $("#erro").html("Não é possivel exibir a questão requisitada!");
                        $("#modalErro").modal("show");
                    }
                }
            });

...
if (!empty($URL[1]) && is_numeric($URL[1])) :
    ...
    $daoAddRecompensa = new DaoPaciente();
    $daoAddRecompensa->showPaciente($URL[1]);
    $add = $daoAddRecompensa->getResult();
    if ($add) :
        $aux = true;
    else :
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            ...
                            <div class="panel-body">
                                ...
                                <form id="form-add-recompensa" class="form-validate form-horizontal" method="post">

                                    <input type="hidden" name="id_paciente" id="id_paciente" value="<?= $add['id_paciente'] ? $add['id_paciente'] : ""; ?>" />


                                    <div class="form-group ">
                                        <label for="link" class="control-label col-lg-2" for="link">
                                            Link da recompensa
                                            <span class="obrigatorio">*</span>
                                        </label>
                                        <div class="col-lg-10">
                                            <input type="text" name="link" id="link" class="form-control" placeholder="Adicione o link do video aqui">
                                        </div>
                                    </div>

                                    <div class="form-group ">
                                        <label for="descricao" class="control-label col-lg-2" for="descricao">
                                            Descrição
                                            <span class="obrigatorio">*</span>
                                        </label>
                                        <div class="col-lg-10">
                                            <input type="text" name="descricao" id="descricao" class="form-control" placeholder="Adicione uma breve descrição da recompensa aqui">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <div class="col-lg-offset-2 col-lg-10">
                                            <button id='add-recompensa' class='btn btn-primary'><span class='state'>Adicionar</span></button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
<script>
    $(document).ready(function() {
        var working = false;
        $("#add-recompensa").on("click", function() {
            $('#form-add-recompensa').submit(function(e) {
                e.preventDefault();
                if ($('#link').val() !== '' && $('#descricao').val() !== '') {
                    if (working)
                        return;
                    working = true;
                    var $this = $(this);
                    ...
                    setTimeout(function() {
                        $.ajax({
                            ...
                            type: "POST",
                            data: $("#form-add-recompensa").serialize() + "&action=addRecompensa",
                            success: function(resultado) {
                                if (resultado == "error") {
                                    ...
                                    $state.html('Cadastrar');
                                    spinner.removeClass('spinner');
                                    $("#msgSucesso").html("Recompensa adicionado com sucesso!");
